/* ================================================================================
   SCRIPT.JS - VERSÃO MEDIEVAL (DESIGNER MELHORADO)
   ================================================================================ */
...
// --- CORES & LISTAS ---
const CORES_TEXTO_RARIDADE = { 'Comum': '#555555', 'Incomum': '#2e8b57', 'Raro': '#00008b', 'Raríssima': '#800080', 'Rarissima': '#800080', 'Épico': '#b8860b', 'Epico': '#b8860b', 'Lendário': '#8b0000', 'Lendario': '#8b0000', 'Mítico': '#008080', 'Mitico': '#008080' };
...
/* --- CRIAÇÃO DO ITEM DE INVENTÁRIO COM NOVO ESTILO (BACKGROUND COLOR) --- */
    ...
    div.innerHTML = ` 
        <div class="inv-header-row"> 
            <div class="inv-left-group"> 
                <input type="checkbox" class="inv-equip-check" ${isEquipped ? 'checked' : ''} title="Equipar/Usar" onchange="salvarAutomaticamente()"> 
                <div> 
            ...
            <div class="inv-contador-group"> 
                ...
                <span class="inv-qtd-display" style="min-width: 25px; text-align: center;">x${dados.qtd}</span> 
                <button onclick="alterarQtdInventario(this.closest('.inv-item'), 1)" class="mod-btn compact-btn">+</button> 
            </div> 
        </div> 
        <div class="inv-desc-display">${dados.desc}</div> 
        <div style="display: flex; justify-content: flex-end; margin-top: 5px;"> 
            <button onclick="if(confirm('Deletar?')) { this.closest('.inv-item').remove(); salvarAutomaticamente(); }" class="mod-btn" style="font-size: 0.7rem; border-color: var(--cor-perigo); color: var(--cor-perigo);">Remover</button> 
        </div> 
    `; 
    container.appendChild(div); 
    tippy(div.querySelector('.inv-nome-display'), { theme: 'translucent', animation: 'scale' });
}
...
function listarFichasNuvem() { 
    ...
    db.collection("fichas").where("uid_dono", "==", usuarioAtual.uid).orderBy("data_atualizacao", "desc").get().then((querySnapshot) => { 
        listaDiv.innerHTML = ""; 
        ...
        querySnapshot.forEach((doc) => { 
            ...
            // FORMATAÇÃO DA DATA
            let dataStr = "";
            if (ficha.data_atualizacao && ficha.data_atualizacao.toDate) {
                const date = ficha.data_atualizacao.toDate();
                dataStr = date.toLocaleString('pt-BR'); // Formato dd/mm/aaaa hh:mm:ss
            }
            ...
            // Adicionado a data abaixo do nome
            item.innerHTML = ` 
                <div>
                    <strong style="color: var(--cor-destaque); display:block;">${ficha.nome_personagem}</strong>
                    <span style="font-size: 0.75rem; color: #666; font-style: italic;">📅 ${dataStr}</span>
                </div> 
                <div> 
        ...
        console.error(error);
        // Tenta buscar sem ordenação se falhar (caso falte índice composto no Firebase)
        db.collection("fichas").where("uid_dono", "==", usuarioAtual.uid).get().then((querySnapshot) => {
             listaDiv.innerHTML = "";
             querySnapshot.forEach((doc) => {
                const ficha = doc.data(); 
                const idDoc = doc.id; 
                ...
                if (ficha.data_atualizacao && ficha.data_atualizacao.toDate) {
                    dataStr = ficha.data_atualizacao.toDate().toLocaleString('pt-BR');
                }
                const item = document.createElement('div'); 
                item.style.borderBottom = "1px solid #444"; item.style.padding = "10px"; item.style.display = "flex"; item.style.justifyContent = "space-between"; item.style.alignItems = "center"; 
                item.innerHTML = ` <div><strong style="color: var(--cor-destaque); display:block;">${ficha.nome_personagem}</strong><span style="font-size: 0.75rem; color: #666; font-style: italic;">📅 ${dataStr}</span></div> <div> <button class="mod-btn compact-btn" onclick="carregarDaNuvem('${idDoc}')" style="border-color: var(--cor-sucesso);">Abrir</button> <button class="mod-btn compact-btn" onclick="deletarDaNuvem('${idDoc}')" style="border-color: var(--cor-perigo);">X</button> </div> `; 
                listaDiv.appendChild(item); 
             });
        });
    }); 
...
function atualizarGrafico() { 
    const ctx = document.getElementById('graficoAtributos'); if (!ctx) return; 
    const dados = [ getAttrValue("Forca"), getAttrValue("Destreza"), getAttrValue("Agilidade"), getAttrValue("Resistencia"), getAttrValue("Espírito"), getAttrValue("Carisma"), getAttrValue("Inteligencia") ]; 
    if (graficoInstance) { graficoInstance.data.datasets[0].data = dados; graficoInstance.update(); return; } 
    graficoInstance = new Chart(ctx, { 
        type: 'radar', 
        data: { labels: ['FOR', 'DES', 'AGI', 'RES', 'ESP', 'CAR', 'INT'], datasets: [{ label: 'Nível', data: dados, backgroundColor: 'rgba(139, 0, 0, 0.4)', borderColor: '#8B0000', borderWidth: 2, pointBackgroundColor: '#B8860B', pointBorderColor: '#2E2315' }] }, 
        options: { scales: { r: { angleLines: { color: 'rgba(0,0,0,0.2)' }, grid: { color: 'rgba(0,0,0,0.1)' }, pointLabels: { color: '#5c0a0a', font: { size: 12, family: 'Cinzel' } }, ticks: { display: false }, suggestedMin: 0, suggestedMax: 10 } }, plugins: { legend: { display: false } } } 
    }); 
}
